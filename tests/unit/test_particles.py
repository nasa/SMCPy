...
def test_set_particles(
    particles, dummy_params, dummy_log_likes, dummy_log_weights, dummy_param_array
):
    ...
    assert particles.attrs == dict(
        {"total_unnorm_log_weight": pytest.approx(2.50258509)}
    )
...
@pytest.mark.parametrize(
    "params, weights",
    (({"a": [1, 2], "b": [2, 3]}, [1, 1]), ({"a": [1, 5 / 3], "b": [4, 2]}, [1, 3])),
)
...
@pytest.mark.parametrize(
    "params, weights, expected_var",
    (
        ({"a": [1, 2], "b": [2, 3]}, [1, 1], {"a": 0.5, "b": 0.5}),
        ({"a": [1, 5 / 3], "b": [4, 2]}, [1, 3], {"a": 2 / 9, "b": 2.0}),
    ),
)
...
def test_compute_covariance(mocker):
    ...
    cov_mock = mocker.patch("smcpy.smc.particles.np.cov", return_value=np.array(1))
